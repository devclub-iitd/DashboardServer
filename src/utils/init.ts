...
import {ADMIN_ENTRY, ADMIN_CASI_EMAIL} from '../utils/secrets';
...
const initAdmin = () => {
  User.findOne({
    ...
    if (doc != null) {
      ...
      if (doc.get('casi_email') != ADMIN_CASI_EMAIL) {
        console.log("Devclub user's CASI email didn't match, setting it.");
        doc.set('casi_email', ADMIN_CASI_EMAIL);
        doc
          .save()
          .then(_ => console.log("Set Devclub user's CASI Email"))
          .catch(err => {
            console.error(err);
            throw new Error('Unable set CASI Email of devclub user');
          });
      } else {
        console.log("Devclub user's CASI Email matches");
      }
